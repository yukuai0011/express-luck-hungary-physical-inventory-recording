...
This repository contains a React Native app (Expo managed) using GlueStack UI. A GitHub Action builds an Android APK on every push and uploads it as a workflow artifact.

## Tech
- React Native (Expo managed)
- GlueStack UI (@gluestack-ui/themed)
- EAS CLI (local build via Docker in GitHub Actions)
...
## CI build output
The workflow generates app-debug.apk and uploads it as an artifact that you can download from the workflow run page.
...
Note: The APK is built in CI using EAS local build; no local Android setup is necessary unless you want to run it on a device locally.
